 ...
 * ******************************************************************************************************************************/
    ...
    while(true){
        ...
        for(int i=0;i<nms_data.size();i++){
            if(nms_data[i].score>0.75){
                //std::cout<<nms_data[i].score<<std::endl;
                TrackingBox cur_box;
                //int x1=int(nms_data[i].x1*w);
                ///int y1=int(nms_data[i].y1*w);
                ///int x2=int(nms_data[i].x2*w);
                //int y2=int(nms_data[i].y2*w);
                cur_box.box.x=int(nms_data[i].x1*w);
                cur_box.box.y=int(nms_data[i].y1*w);
                cur_box.box.width=int(nms_data[i].x2*w);
                cur_box.box.height=int(nms_data[i].y2*w);
                cur_box.id=i;
                cur_box.frame=frame_id;
                detFrameData.push_back(cur_box);
                std::vector<TrackingBox> tracking_results = sorter.update(detFrameData);
                //std::cout<<x1<<" "<<y1<<" "<<x2<<" "<<y2<<std::endl;
                //cv::rectangle(inputimage,cv::Rect(x1,y1,x2-x1,y2-y1),cv::Scalar(0,255,0),4);
                for (TrackingBox it : tracking_results) {
                cv::rectangle(inputimage,cv::Rect(it.box.x, it.box.y,it.box.width-it.box.x,it.box.height-it.box.y) , sorter.randColor[it.id % 20], 2,8,0);
                //cv::Mat face_photo = inputimage(Rect(it.box.y, it.box.x, it.box.height - it.box.y, it.box.width - it.box.x));
